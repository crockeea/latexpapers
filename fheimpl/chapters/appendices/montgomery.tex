...
\subsection{Stricter Constraints}
First, let's throw out negative numbers so that we can us unsigned numbers. Then we can set $R=2^{64}$, and the Montgomery reduction becomes even more efficient, as we don't need any masking. Each term in the line 3 of algorithm 1 are in $[0,N\cdot R]$, so we need $N\cdot R \le 2^{127}$ so that when we add the terms, we don't overflow. This implies that $N\le 2^{63}$. Another way to see this requirement is that Montgomery reduction outputs in the range $[0,2N)$, which is $[0, 2^{64})$.

However, this extra space comes at a cost: we cannot represent negative numbers, and have to take special care to avoid them. Specifically, we have to reduce the first argument to $[0,q)$, add $q$ to it, reduce the second argument to $[0,q)$, and subtract it from the first. The output is now in $[0,2q)$. Addition is similarly convoluted: we have to reduce both arguments to $[0,q)$ before adding. We also have to take care that we don't ever multiply two values in $[0, 2q)$ together; these could overflow. Instead, we need to reduce at least one of the inputs to $[0,q)$ first.
