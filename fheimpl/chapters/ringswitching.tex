...
\begin{document}
	...
	We now describe the high-level structure of ring-switching. Let $m(X)$ be an encoded plaintext polynomial in a ring of dimension $N=2^\ell$, and let $n=2^k$ such that $k<\ell$. Define the \emph{index} as $r:=[\mathcal{R}_N : \mathcal{R}_n]=N/n$. Define an operation $\mathsf{Break}_n(\cdot)$ that, on input a polynomial $d(X)=\sum_{j=0}^{N-1}c_j\cdot X^j$, outputs a set of polynomials $d_i(X)=\sum_{j=0}^{n-1} c_{j\cdot r+i}\cdot X^j$ for $r=N/n$ and $i\in[0, r)$. We can write $d(X)=\sum_{i=0}^{r-1} d_i(X^r)\cdot X^i$. We note that $\set{X^i}_{i\in[0,r)}$ is the \emph{relative power basis} of $\mathcal{R}_N=\Z[X]/(X^N+1)$ over $\mathcal{R}_n=\Z[Y]/(Y^n+1)$~\cite{cryptoeprint:2015/1134}. This means that we can write an element of $\mathcal{R}_N$ as a polynomial (in $X$) of degree $<r$ with coefficients in $\mathcal{R}_n$, where $Y=X^r$.
	...
	\subsection{Simple}
	Let $\Xi_{2^k}(X) = X^{2^{k-2}} - i$ be the Gaussian counterpart to $\Phi_{2^k}(X)$.
	
	Concretely, we will switch from $R=\Z[i][Z]/(\Xi_{2^{14}}(Z))$ to $S=\Z[i][X,W]/(\Xi_{2^{10}}(X), \Phi_{17}(W))$. Note that we will primarily use $i$ as an index and not as $\sqrt{-1}$; uses should be clear from context. Both rings hold $2^{12}$ slots.
	
	Let $p=17$, $n=256$, $\zeta_i = \zeta^{5^i}$ where $\zeta$ is a $4n\th$ root of unity, $\eta$ is a $17\th$ root of unity, and $\xi_i=\xi^{5^i}$ where $\xi$ is a ${2^{14}}\th$ root of unity.
	
	We will view messages in $R$ as corresponding to a tensor with shape $(256, 16)$.
	
	Let $E=\Z[i][X]/(\Xi_{2^{10}}(X)) = R \cap S$ be the largest common subring of $R$ and $S$. The $E$-basis for $R$ is ${1, Z, Z^2, \ldots, Z^{15}}$. We can write any element $r\in R$ as $r=\sum_{i=0}^{15} e_i(Z^{16})\cdot Z^i$, where $e_i\in E$.
	
	Let $r\in R$ and $s=L(r)$. Our goal is $L\parens*{r\parens*{\xi^{5^j}}} = s\parens*{\zeta^{j\bmod 256}, \eta^{\floor{j/256}}}$.  Per~\cite{cryptoeprint:2015/1134}, the function $L: R\rightarrow S$ is completely determined by how $L$ acts on the $E$-basis of $R$.
	\begin{align*}
		L\parens*{r\parens*{Z}} &= L\parens*{\sum_{i=0}^{15} e_i\parens*{X}\cdot Z^i} \\
		  &= \sum_{i=0}^{15} e_i\parens*{X}\cdot L\parens*{Z^i} \\
		  &= \sum_{i=0}^{15} e_i\parens*{X}\cdot \ell_i(X, W)
	\end{align*}
	...
	\section{Ring Tunneling}
	$\Lambda\circ\lambda$~\cite{cryptoeprint:2015/1134} introduced ring-tunneling as an efficient way to switch between different cyclotomic rings via their greatest common subring. The paper introduces ring-tunneling in the context of BGV homomorphic encryption; we give a adaptation for CKKS here. In this section, let $R$ be the source ring with cyclotomic index $m$, $S$ be the target/destination ring with cyclotomic index $m'$, and $E$ be the cyclotomic ring with index $\gcd(m, m')$. Then $E\subseteq R$ and $E\subset S$. An $E$-basis of $R$ is a set of $\frac{m}{\gcd(m, m')}$ elements $\set{v_i}$ of $R$ such that any element $r\in R$ can be written as $\sum_{i} e_i\cdot v_i = \sum_{i} \mathrm{embed}_{E\rightarrow R}(e_i)\cdot v_i$ for some elements $e_i\in E$ specific to $r$.
	
	Ring tunneling allows us to induce an arbitrary $E$-linear function $L$. A function is $E$-linear if $L(r+r') = L(r) + L(r')$ for all $r, r'\in R$ and if $e\cdot L(r) = L(e\cdot r)$ for all $e\in E, r\in R$.
	
	In an offline phase, we computing $L$ on the $E$-basis of $R$. In the online phase, we decompose an input $r$ into its $E$-basis coefficients, the recombine them with the translated $E$-basis into an element of $S$:
	\[L(r) = L\parens*{\sum_i e_i \cdot v_i} = \sum_i e_i\cdot L(v_i)\]
	by the definition of $L$ being $E$-linear.
	
	In the general setting, this operation must coincide with a keyswitch from a key in $R$ to a key in $S$, so the entire process can be computed via a modified key-switch. Specifically, we compute key-switch keys for $s_\mathrm{in}\cdot L(v_i)\in R$ to $s_\mathrm{out}$. In the online phase, given a ciphertext $(a, b)$:
	\begin{enumerate}
		\item Decompose $a$ and $b$ into their $E$-basis coefficients $\set{a_i}$ and $\set{b_i}$.
		\item Compute $b' = L(b) = \sum_i b_i\cdot L(v_i)$.
		\item Compute $a'_i = \mathrm{embed}_{E\rightarrow S}(a_i)$ and $b'_i = L(v_i)\cdot \mathrm{embed}_{E\rightarrow S}(b_i)$.
		\item Apply the special key-switch to $(a'_i, b'_i)$. The output decrypts to $a'_i\cdot L(v_i)\cdot s_\mathrm{in} + L(v_i)\cdot b_i$
		\item Sum up the result.
	\end{enumerate}
	
	In the context of CKKS, ring-tunneling requires rescaling because we typically think about ring-tunneling as it affects the slots, so $L(v_i)$ is typically an encoded ring element which includes a scale factor $\Delta$. Therefore, after the steps above, we would apply a rescale.
	
	As an example, we can consider a setting where $R=E$ and we have several $R$ elements that we wish to combine into a single $S$ element. In this setting, the $E$-basis of $R$ is the singleton $\set{1}$. Note that embedding both components of a ciphertext into $S$ results in replicating the slots of the input across all the slots of the output by concatenating adjacent copies of the input slots. We want to mask out a specific copy of the input slots and add it to the masked slots of the other pieces. It's clear that $L(v_i)$ should encode the message $[0, 0, 0, \ldots, 1, 1, \ldots 1, 0, 0, \ldots 0]$.
