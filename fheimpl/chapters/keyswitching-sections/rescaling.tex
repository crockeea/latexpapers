...
Using~\cref{sec:fastscaling}, we can achieve rescaling in $\mathcal{O}(\abs{\mathcal{C}}+\abs{\mathcal{B}}\cdot \abs{\mathcal{C}})$. We should prefer this over FBC, especially since $\abs{\mathcal{C}}$ is typically very small.
...
\label{sec:rescale_error}
Given a ciphertext $(a,b)$ encrypting a message $m$, our true goal is to output an encryption of $m/q$ for some $q$. Clearly, $\parens*{\frac{a}{q}, \frac{b}{q}}$ is what our new ciphertext should be. However, these don't have integral coefficients, so we instead output $\parens*{a'=\round*{\frac{1}{q}\cdot a}=\parens*{\frac{1}{q}\cdot a}+e^\mathsf{round}_a, b'=\round*{\frac{1}{q}\cdot b}=\parens*{\frac{1}{q}\cdot b}+e^\mathsf{round}_b}$, where the error terms have coefficients in $[-1/2, 1/2]$ if rescaling with canonical representatives.

Initially, $a\cdot s+b\approx m+e_\mathsf{CT}$. After rescaling, 
\begin{align}
	a'\cdot s+b' &=\parens*{\parens*{\frac{1}{q}\cdot a}+e^\mathsf{round}_a}\cdot s + \parens*{\frac{1}{q}\cdot b}+e^\mathsf{round}_b \\
	&= \parens*{\frac{1}{q}\cdot a}\cdot s + \parens*{\frac{1}{q}\cdot b} + e^\mathsf{round}_a\cdot s +e^\mathsf{round}_b \\
	&\approx  \parens*{\frac{1}{q}\cdot m} + \frac{e_\mathrm{CT}}{q} + e^\mathsf{round}_a\cdot s +e^\mathsf{round}_b
\end{align}

We generally ignore the isolated $e^\mathsf{round}_b$ term since it is small relative to $e^\mathsf{round}_a\cdot s$. As long as $s$ is small (which is typically the case), then this term doesn't contribute much to the overall error, either.
...
When performing nearest-integer rounding, $e^\mathsf{round}$ has coefficients in $[-\frac{1}{2}, \frac{1}{2})$. When performing floor rounding, $e^\mathsf{round}$ has coefficients in $[0, 1)$, hence using floor essentially doubles the rounding error compared to using nearest-integer.
