...
Roughly, the key-switching key $ksk$ is an LWE ciphertext encrypting $s_1$ (the input key) under $s_2$ (the output key):
...
Broadly, the idea is to ``decompose'' a ring element $c$ into a sum $\sum_{i=0}^t c_i D_i$, where $c_i$ has small coefficients, and $D_i$ is arbitrary. We explore two ways to do this below.
...
\label{sec:ks-crt-decomposition}
...
One way to use this formula is to let each piece be $\abs{x}_{q_i}\cdot \tilde{Q}_i$, since both terms are in $\Z_{q_i}$, and scale the secret key by $Q_i^*$~\cite{cryptoeprint:2019/688}. The result is similar to the base-$b$ decomposition: we decompose $x$ into pieces with small coefficients. To decompose, we simply multiply each RNS component by a scalar, and encrypt $Q_i^* \cdot s$.

However, we can do better by noting letting the small piece be \emph{just} $\abs{x}_{q_i}$ and scaling the secret key by $\tilde{Q}_i\cdot Q_i^*$~\cite{cryptoeprint:2020/1203}. This is better in two ways: first, the decomposition step is free: there's no scaling. It's also easier when constructing the key-switch key, which involves scaling [each RNS component $s_{1,j}$ of] $s_1$: 
\[\tilde{Q}_i\cdot Q_i^*\cdot s_j = \begin{cases}
	s_{1,j} & j = i \\
	0 & j \ne i 
\end{cases}
\]

If each $q_i$ is roughly $\nu$ bits, then this yields the same noise growth as a radix-$2^\nu$ decomposition. In practice, each key-switch key encrypts $\tilde{Q}_i\cdot Q_i^*\cdot s_1$, and the decomposition of the input is free.
...
\end{align*}
Note that this is the same as decrypting the original ciphertext under $s$, plus an error term $\round*{\frac{c_2}{P}\cdot e} + e_\mathrm{round}$. The coefficients of $c_2$ are independent of $P$, hence by making $P$ larger, we can reduce the affect of that term. Also note that $e_\mathrm{round}$ is very small, with coefficients in \enote{?? $\round*{\frac{c_2}{P}\cdot a}\cdot s - \round*{\frac{c_2}{P}\cdot a\cdot s}$ could be very large...}. 
